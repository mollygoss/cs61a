I"""A Scheme interpreter and its read-eval-print loop."""
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 9A
        targetnew = target.first
        if scheme_symbolp(targetnew):
            arguments = Pair(target.second, expressions.second)
            lambdaexpr = Pair("lambda", arguments)
            env.define(targetnew, scheme_eval(lambdaexpr, env))
            return targetnew
        # END Question 9A
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        # BEGIN Question 15A
            ...
            condition = scheme_eval(clause.first, env)
        if scheme_true(condition):
            if clause.second is nil:
                return condition
            return eval_all(clause.second, env)
        if scheme_false(condition) and expressions.second is None:
            return None
        # END Question 15A
...
def make_let_frame(bindings, env):
    ...
    for b in range(len(bindings)):
        if check_form(bindings.first, 2, 2):
            raise SchemeError
        if type(bindings.first.first) is int:
            raise SchemeError
        names = Pair(bindings.first.first, names)
        values = Pair(scheme_eval(bindings.first.second.first, env), values)
        childframe = env.make_child_frame(names, values)
...
def check_formals(formals):
    ...
    # BEGIN Question 11B
    lst = []
    def help(formals, lst):
        if formals == nil:
            return
        if scheme_symbolp(formals.first) and formals.first not in lst:
            lst.append(formals.first)
            return help(formals.second, lst)
        else:
            raise SchemeError 
    return help(formals, lst)
    # END Question 11B
